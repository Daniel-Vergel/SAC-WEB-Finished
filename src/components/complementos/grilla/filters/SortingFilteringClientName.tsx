...
import { PiSortDescending, PiSortAscendingLight } from 'react-icons/pi';
import { useDebounce } from '../../../../utils/useDebounce';
import { setSelectedClientName } from '../../../../store/grillaState.tsx/SelectedClientName';
import { RootState } from '../../../../store/store';


interface SortingFilteringBoxProps {
  requestSort: (column: string, direction: 'asc' | 'desc') => void;
}

const SortingFilteringClientName: React.FC<SortingFilteringBoxProps> = ({ requestSort }) => {
  const dispatch = useDispatch();
  const { clientNameState } = useSelector((state: RootState) => state.selectedClientName);
  const [isOpen, setIsOpen] = useState(false);
  const sortingFilteringRef = useRef<HTMLDivElement>(null);
  const [direction, setDirection] = useState<'asc' | 'desc'>('asc');
 
  const [inputFocused, setInputFocused] = useState(false);
  const [inputValue, setInputValue] = useState(clientNameState);
  const debouncedClientName = useDebounce(inputValue, 2000);

  useEffect(() => {
    dispatch(setSelectedClientName(debouncedClientName));
  ...
  const handleSortBy = (direction: 'asc' | 'desc') => {
    ...
    requestSort('clientName', direction);
  };
...
export default SortingFilteringClientName;
